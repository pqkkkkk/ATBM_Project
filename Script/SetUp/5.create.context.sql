...
BEGIN
    ...
    IF userId = 'ADMIN' THEN
        DBMS_SESSION.SET_CONTEXT('X_UNIVERSITY_CONTEXT', 'IS_ADMIN', 1);
    ELSE
        DBMS_SESSION.SET_CONTEXT('X_UNIVERSITY_CONTEXT', 'IS_ADMIN', 0);
    END IF;
...
END;
/
CREATE OR REPLACE TRIGGER ON_UPDATE_DANGKY_TABLE
  BEFORE UPDATE ON X_ADMIN.DANGKY
  FOR EACH ROW
BEGIN
  IF :OLD.MASV <> :NEW.MASV THEN
    RAISE_APPLICATION_ERROR(-20003, 'Không được phép thay đổi thuộc tính MASV của bảng DANGKY');
  ELSIF :OLD.MAMM <> :NEW.MAMM THEN
    RAISE_APPLICATION_ERROR(-20004, 'Không được phép thay đổi thuộc tính MAMM của bảng DANGKY');
  END IF;
END;
/
COMMIT;