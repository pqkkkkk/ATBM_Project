-- Tạo view trên tất cả các thuộc tính của bảng MOMON các dòng liên quan đến chính bản thân mình cho vai trò GV 
    CREATE OR REPLACE VIEW X_ADMIN.view_GV_MOMON AS 
    SELECT * FROM X_ADMIN.MOMON where MaGV = SYS_CONTEXT('X_UNIVERSITY_CONTEXT','USER_NAME');
...
-- Hàm tạo view trên bảng MOMON của tất cá các môn thuộc học kì hiện tại của năm học đang diễn ra
    CREATE OR REPLACE VIEW X_ADMIN.view_PDT_MOMON AS
...
-- Tạo view xem phân công giảng dạy của các GV thuộc đơn vị của mình cho TRGDV
    CREATE OR REPLACE VIEW X_ADMIN.view_TRGDV_MOMON AS 
    SELECT m.* FROM X_ADMIN.MOMON m JOIN NHANVIEN nv ON m.MAGV = nv.MANV
    WHERE nv.MADV IN (
        SELECT nv2.MADV             
        FROM X_ADMIN.NHANVIEN nv2 
        WHERE nv2.MANV = SYS_CONTEXT('X_UNIVERSITY_CONTEXT','USER_NAME')
    );
    -- Cấp quyền SELECT trên view
    GRANT SELECT ON X_ADMIN.view_TRGDV_MOMON TO XR_TRGDV;


-- Tạo view gồm tất cả các mở môn thuộc khoa của sinh viên đang theo học
    CREATE OR REPLACE VIEW  X_ADMIN.view_SV_MOMON AS
    ...
    WHERE hp.MADV IN (
        ...
        WHERE MASV = SYS_CONTEXT('X_UNIVERSITY_CONTEXT','USER_NAME')
    ) ;
    -- Cấp quyền SELECT trên view cho SV
    GRANT SELECT ON X_ADMIN.view_SV_MOMON TO XR_SV;
